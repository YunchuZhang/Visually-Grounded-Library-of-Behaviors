...
class PolicyCompressor:
    ...
    def __init__(self,  config:Config, initial_policies=[], output_file="", run_name=""):
        ...
        self.init_clusters_with_policyname()


    def init_clusters_with_policyname(self):
        for policy in self.policy_bank:
            #cluster_id = self.num_clusters
            cluster_name = policy.policy_name
            self.clusters[cluster_name] =dict()
            self.clusters[cluster_name]['objects'] = []
            self.clusters[cluster_name]['expert_name'] = policy.policy_name
            self.clusters[cluster_name]['expert'] = policy #.policy_model
            self.num_clusters += 1
    ...
    def add_object(self, obj):
        ...
        success_rates =  [float(x) for x in obj.success_rates_over_class.split(" ")]

        print("pick cluster")
        best_cluster_id = np.argmax(success_rates)
        sc = success_rates[best_cluster_id]

        possible_ids = [id_ for id_ in range(self.num_clusters) if success_rates[id_] == sc]
        best_cluster_id = np.random.choice(possible_ids)

        print(f"pick cluster c{best_cluster_id} with success_rate {success_rates[best_cluster_id]}")


        # pick up the cluster
        cluster = self.clusters[f"c{best_cluster_id}"]
        ...
        #            stats = cluster['expert'].run_forwards(env, obj=obj, num_rollouts=self.num_rollouts, path_length=self.max_path_length, render=self.config.render)#, accept_threshold=self.accept_threshold)
        #
