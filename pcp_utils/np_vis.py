...
def get_pcd_object(points, clip_radius=2., debug=False):
    """
    points: npoint3
    """
    inliers = get_inliers(points, clip_radius, debug=debug)
    if debug:
...
def visualize(points_list, clip_radius=2., frame=True, coord_size=0.8):
    """
    # convert all points to pcds and visualize using open3d

    points_list: [npoint x 3] * #pointclouds
    frame: whether to print the xyz coordinate frame
    coord_size: size for this xyz coordinate visualization
    """
    pcds = list()
    for points in points_list:
        pcd = get_pcd_object(points, clip_radius)
    ...
    # create mesh frame
    if frame:
        frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=coord_size)
        pcds.append(frame)
...
def get_inliers(points, clip_radius, debug=False):
...
def unproject_depth(depths, intrinsics, origin_T_camXs,
    ...
    xyz_origins = list()
    xyz_camRs = list()
    # if camR_T_origin is not None else True
    for i in range(len(depths)):
        xyz_camX = vectorized_unproject(depths[i], intrinsics[i])
        # if mujoco_py_scene\
        ...
        xyz_origin = np.dot(origin_T_camXs[i], xyz_camX_homogenous.T).T
        xyz_origins.append(xyz_origin[:,:3])
        ...
        if camR_T_origin is not None:
            ...
            xyz_camRs.append(xyz_camR[:, :3])
        else:
            xyz_camR = xyz_origin[:, :3]
            xyz_camRs.append(xyz_camR)

    output_imgs = dict()
    if do_vis:
        img_xyz = visualize(xyz_camXs, clip_radius=clip_radius, frame=vis_frame)
        img_origin = visualize(xyz_origins, clip_radius=clip_radius, frame=vis_frame)
        img_camR = visualize(xyz_camRs, clip_radius=clip_radius, frame=vis_frame)

        output_imgs['img_xyz'] = img_xyz
        output_imgs['img_origin'] = img_origin
        output_imgs['img_camR'] = img_camR

    return output_imgs, xyz_origins, xyz_camRs
...
def get_single_value_scale(scale_str):
...
def get_meshes_from_xml(xml_path):
    ...
    for asset in root.find('asset'):
         if asset.tag == "mesh":
            asset_name = asset.attrib['name']
            scale = [float(s) for s in asset.attrib['scale'].split(" ")]
            asset_mesh[asset_name] = (asset.attrib['file'], scale)

    # find the object called collision
    obj_body = root.find('worldbody').find('body')
    ...
    obj_geoms = dict()
    obj_native_id = 0
    for geom in obj_collision_body.findall('geom'):
       geom_type = geom.attrib['type']

       if geom_type == "mesh":
           if geom.attrib['mesh'] not in asset_mesh:
                print("cannot find mesh for geom", geom.attrib['mesh'], "please check your file", xml_path)
           obj_name = geom.attrib['mesh']
       else:
           obj_name = f"geom_type_{obj_native_id}"
           obj_native_id += 1
       obj_geoms[obj_name] = dict()
       obj_geoms[obj_name]['type'] = geom_type
       if 'pos' in geom.attrib:
           obj_geoms[obj_name]['pos'] = [float(s) for s in  geom.attrib['pos'].split(" ")]
       else:
           obj_geoms[obj_name]['pos'] = [0, 0, 0]
       if 'size' in geom.attrib:
           obj_geoms[obj_name]['size'] = [float(s) for s in  geom.attrib['size'].split(" ")]
       else:
           obj_geoms[obj_name]['size'] = [1, 1, 1]
       if 'euler' in geom.attrib:
           obj_geoms[obj_name]['xmat'] = transformations.euler_matrix(*[float(s) for s in geom.attrib['euler'].split(" ")])

    return asset_mesh, obj_geoms



def compute_bounding_box_from_table_subtract(all_xyz_camR, table_height_R, axis=2, keep_larger=True):

    xyz_camR_set_boundary = all_xyz_camR
    if keep_larger:
        xyz_camR_set_boundary = xyz_camR_set_boundary[xyz_camR_set_boundary[:,axis] > table_height_R, :]
    else:
        xyz_camR_set_boundary = xyz_camR_set_boundary[xyz_camR_set_boundary[:,axis] < table_height_R, :]
    #xyz_camR_set_boundary = xyz_camR_set_boundary[np.abs(xyz_camR_set_boundary[:,1]) < 0.2, :]
    #xyz_camR_set_boundary = xyz_camR_set_boundary[np.abs(xyz_camR_set_boundary[:,2]) < 0.2, :]

    bbox_points = pcp_utils.np_vis.compute_bounding_box_from_pcs(xyz_camR_set_boundary)

    return bbox_points

def compute_bounding_box_from_obj_xml(xml_path, object_xpos, object_xmat=None, scale=1.0, euler=None, debug=False,
                                      return_combined_mesh=False):
    """
    fits a mesh to point cloud of the object
    :param xml_path: string indicating xml path
    :param object_xpos: (3,) position of the object
    object_xmat: [9]
    :return: None
    """

    asset_mesh, obj_geoms = get_meshes_from_xml(xml_path)
    ...
    for geom_name, geom in obj_geoms.items():
        ...
        if euler is not None:
            ...
            if 'xmat' not in geom:
                current_euler = [0, 0, 0]
                dalpha, dbeta, dgamma = current_euler
                rot_mat = transformations.euler_matrix(dalpha, dbeta, dgamma)
            else:
                rot_mat = geom['xmat']
            dalpha, dbeta, dgamma = euler
            d_rot_mat = transformations.euler_matrix(dalpha, dbeta, dgamma)
            new_xmat = np.matmul(d_rot_mat, rot_mat)
        ...
        if geom['type'] == "mesh":
            ...
            if "xmat" in geom:
                mesh_scaled = mesh_scaled.apply_transform(geom['xmat'])
    ...
    # meshes = [trimesh.load(f) for f in mesh_paths]
    combined_mesh = np.sum(meshes)
    #if not is_native_geom:
    combined_mesh = combined_mesh.apply_scale(scale)


    #scaled_combined_mesh = combined_mesh.apply_scale(scale)

    if debug:
        return {"bounind_box_no_trans": combined_mesh.bounding_box.bounds}


    #print("object_box", combined_mesh.bounding_box.bounds)
    object_xmat_final = np.eye(4, dtype=np.float32)
    if object_xmat is not None:
        object_xmat_final[:3, :3] = np.reshape(object_xmat, [3, 3])
    combined_mesh = combined_mesh.apply_transform(object_xmat_final)
    combined_mesh = combined_mesh.apply_translation(object_xpos)
    mesh_bbox = combined_mesh.bounding_box
    bbox_bounds = copy.deepcopy(mesh_bbox.bounds)
    bbox = compute_bounding_box_from_pcs(bbox_bounds)

    if return_combined_mesh:
        return bbox, combined_mesh
    else:
        return bbox


def get_bbox_attribs(bbox):
    """
    bbox: 8x3
    """
    min_bounds = np.min(bbox, axis=0)
    max_bounds = np.max(bbox, axis=0)

    extents = max_bounds - min_bounds
    center = min_bounds + (max_bounds - min_bounds) / 2.0
...
def make_lineset_from_rot(center, extent):
...
def make_lineset_4x4matrix(rot, length=0.02):
    """
    rot: 4x4 matrix
    """
    origin = rot[:3, 3]
    x_axis = origin + rot[:3, 0] * length
    y_axis = origin + rot[:3, 1] * length
    z_axis = origin + rot[:3, 2] * length

    points = np.stack([origin, x_axis, y_axis, z_axis], axis=0)
...
def compute_bounding_box_from_pcs(pts):
    """
    pts: npts x 3
    """
