...
class GraspParamsController(Policy):
    class Config(Config):
    ...
    def get_bbox_properties(self, env, obj_info):
        ...
        if self.detector is None: # use gt
            ...
            self.detector.detect_objects(env)
    ...
    def run_forwards(self, env, num_rollouts, path_length=None, obj=None, render=False, accept_threshold=0, cluster_name="", place = False):
        """
        ...
        for iter_id in range(num_rollouts):
            ...
            if self.save_video:
                self.dump_video(f"tmp/{obj_info.name}_{iter_id}_{success}_{cluster_name}.mp4")
...
#            cur_reward.append(reward)
    ...
    def goToGoal(self, env, lastObs, mesh_obj_info=None):
        ...
        if self.place:
            ...
            run_one = True
            while  np.linalg.norm(goal - bbox_xpos) >= 0.01 and timeStep <= self.max_path_length:
                print(timeStep,self.max_path_length)
                #print(np.linalg.norm(goal - bbox_xpos), goal_pos_for_gripper, gripper_position)
                if run_one: # first time, go a rroughly toward the goal
                    thres = 0.01
                else:
                    thres = 0.002
                rewards, timeStep = self.gripper_goto_pos_orn(env, goal_pos_for_gripper, new_gripper_quat, goal_current_thres= thres, speed=3, open=False, timeStep=timeStep, mesh_obj_info=mesh_obj_info)
                run_one=False
                # cur_reward += rewards
                # if cur_reward[-1] > 0:
                #     break
