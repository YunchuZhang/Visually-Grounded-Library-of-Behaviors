...
def eye_3x3(B):
    rt = torch.eye(3).view(1,3,3).repeat([B, 1, 1])
    return rt

def eye_3x3s(B, S):
    rt = torch.eye(3).view(1,1,3,3).repeat([B, S, 1, 1])
    return rt
...
def eye_4x4s(B, S):
    rt = torch.eye(4).view(1,1,4,4).repeat([B, S, 1, 1])
    return rt
...
def merge_rt(r, t):
    ...
    assert(D==3)
    t = t.view(B, 3)
    rt = torch.eye(4).unsqueeze(0).repeat([B, 1, 1])
    rt[:,:3,:3] = r
    rt[:,:3,3] = t
    return rt
...
def pack_intrinsics_py(fx, fy, x0, y0):
    B = list(fx.shape)[0]
    K = np.zeros((B, 3, 3))
    K[:, 0, 0] = fx
    K[:, 1, 1] = fy
...
def scale_intrinsics(K, sx, sy, py=False):
    fx, fy, x0, y0 = split_intrinsics(K)
    fx = fx*sx
    fy = fy*sy
    x0 = x0*sx
    y0 = y0*sy
    K = pack_intrinsics(fx, fy, x0, y0) if not py else\
        pack_intrinsics_py(fx, fy, x0, y0)
    return K
...
def pack_intrinsics(fx, fy, x0, y0):
    B = list(fx.shape)[0]
    K = torch.zeros(B, 4, 4, dtype=torch.float32, device=DEVICE)
    K[:,0,0] = fx
    K[:,1,1] = fy
    K[:,0,2] = x0
    K[:,1,2] = y0
...
def depth2pointcloud(z, pix_T_cam, device=DEVICE):
    B, C, H, W = list(z.shape)  # this is 1, 1, H, W
    y, x = utils_basic.meshgrid2D(B, H, W, device=device)
...
def convert_boxlist_to_lrtlist(boxlist):
    ...
    # NOTE: rtlist converts from box coordinate to ref coordinates
...
def convert_box_to_ref_T_obj(box3D):
    ...
    # box3D is B x 9
    x, y, z, lx, ly, lz, rx, ry, rz = torch.unbind(box3D, dim=1)
    rot0 = torch.eye(3).unsqueeze(0).repeat(B, 1, 1)  # no rotation
    tra = torch.stack([x, y, z], dim=1)
    center_T_ref = merge_rt(rot0, -tra)  # actually transforms points from cam to box
    # center_T_ref is B x 4 x 4

    t0 = torch.zeros([B, 3])
    rot = eul2rotm(rx, -ry, -rz)  # ask Adam about the orientation of the box
    ...
    # transforms the cam points to box points
    ...
    # NOTE: converting obj corners to cam coords
    ref_T_obj = obj_T_ref.inverse()
    # converts box coordinates to camX coordinates
    return ref_T_obj
...
def transform_boxes_to_corners_single(boxes, device=DEVICE):
    N, D = list(boxes.shape)
    assert(D==9)

    xc,yc,zc,lx,ly,lz,rx,ry,rz = torch.unbind(boxes, dim=1)
    # these are each shaped N

    ref_T_obj = convert_box_to_ref_T_obj(boxes)

    xs = torch.stack([-lx/2., -lx/2., -lx/2., -lx/2., lx/2., lx/2., lx/2., lx/2.], dim=1)
    ys = torch.stack([-ly/2., -ly/2., ly/2., ly/2., -ly/2., -ly/2., ly/2., ly/2.], dim=1)
    zs = torch.stack([-lz/2., lz/2., -lz/2., lz/2., -lz/2., lz/2., -lz/2., lz/2.], dim=1)

    xyz_obj = torch.stack([xs, ys, zs], dim=2)
...
def transform_boxes_to_corners(boxes, device=DEVICE):
    ...
    boxes_ = __p(boxes)
    corners_ = transform_boxes_to_corners_single(boxes_, device)
...
def convert_corners_to_axis_aligned_boxlist(corners):
    B, N, E, D = list(corners.shape)
    assert(D == 3)
...
def apply_4x4_to_corners(Y_T_X, corners_X, device=DEVICE):
