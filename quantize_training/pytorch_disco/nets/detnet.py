...
def anchor_deltas_to_bboxes(anchor_deltas, indices, config):
    ...
    object_center = grid_center + anchor_deltas[:, :3] * config.det_anchor_size
    object_min = object_center - 0.5 * torch.exp(anchor_deltas[:, 3:]) * config.det_anchor_size
    object_max = object_center + 0.5 * torch.exp(anchor_deltas[:, 3:]) * config.det_anchor_size
...
def rpn_proposal_graph(pred_objectness, pred_anchor_deltas, valid_mask, corners_min_max_g, iou_thresh=0.5, config=None,
    device=None): #tested
    ######################## ROI generation ####################
    ...
    if len(high_prob_indices > 0):
        for i in list(range(B)):
            ...
            corners_min_max_g, valid_mask, pred_objectness, pred_anchor_deltas, iou_thresh=iou_thresh, config=config, device=device)
...
def detection_target_graph(i, high_prob_indices, corners_min_max_g, valid_mask, pred_objectness, pred_anchor_deltas,
                           iou_thresh=0.5, config=None, device=None): #tested
    ...
    batch_i_bboxes, batch_i_bboxes_co = anchor_deltas_to_bboxes(
        batch_i_anchor_deltas, batch_i_indices[:,1:], config)
    # N x 3 x 2 and N x 6
...
class DetNet(nn.Module):
    def __init__(self, config):
        ...
        if self.config.deeper_det:
            self.resnet = ResNet3D(in_channel=32, pred_dim=32).to(self.device)
        self.pred_dim = 7
        self.conv1 = torch.nn.Conv3d(in_channels=self.config.feat_dim, out_channels=self.pred_dim, kernel_size=3, stride=1, padding=1).to(self.device)
    ...
    def forward(self,
        ...
        # st()
        total_loss = torch.tensor(0.0).to(boxes_g.device)
        ...
        corners = utils.geom.transform_boxes_to_corners(boxes_g) # corners is B x N x 8 x 3, last dim in xyz order
        ...
        N = self.config.K
        boxes_g = boxes_g[:, :N, :6]  # first 6 is taken, meaning no rotation, its okay since its 0 for me, i.e axis aligned
        corners_min_max_g = corners_min_max_g[:,:N]
        scores_g = scores_g[:, :N] # B x N

        # boxes_g is [-0.5~63.5, -0.5~15.5, -0.5~63.5]  # dont understand this comment
        ...
        # this is computing distance of center of ground truth bounding box from each grid position
        ...
        delta_positions = delta_positions_raw / self.config.det_anchor_size
        ...
        delta_lengths = torch.log(lengths_g / self.config.det_anchor_size)
        ...
        loss_prob = torch.sum(self.config.alpha_pos * cls_pos_loss + self.config.beta_neg * cls_neg_loss)
        ...
        loss_reg = torch.sum(loss_l1/pos_equal_one_sum_safe.view(-1, 1, 1, 1, 1))/self.config.B

        if summ_writer:
            total_loss = utils.misc.add_loss('det/detect_prob', total_loss, loss_prob, self.config.det_prob_coeff, summ_writer)
            total_loss = utils.misc.add_loss('det/detect_reg', total_loss, loss_reg, self.config.det_reg_coeff, summ_writer)
        ...
        (
            ...
            bs_overlaps,
                               ...
                               iou_thresh=0.2, config=self.config, device=boxes_g.device)
        # these are lists of length B, each one leading with dim "?", since there is a variable number of objs per frame

        N = self.config.K * 3
        tidlist = torch.linspace(1.0, N, N).long().to(boxes_g.device)
        tidlist = tidlist.unsqueeze(0).repeat(B, 1)
        padded_boxes_e = torch.zeros(B, N, 9).float().to(boxes_g.device)
        padded_scores_e = torch.zeros(B, N).float().to(boxes_g.device)
        if bs_selected_boxes_co is not None:
            ...
            for b in list(range(B)):
                ...
                padded_boxes0_e = torch.cat([padded_boxes0_e, torch.zeros([1, N, 6], device=boxes_g.device)], dim=1) # 1 x ? x 6
                padded_scores0_e = torch.cat([padded_scores0_e, torch.zeros([1, N], device=boxes_g.device)], dim=1) # pad out

                padded_boxes0_e = padded_boxes0_e[:,:N] # clip to N
                padded_scores0_e = padded_scores0_e[:,:N] # clip to N

                padded_boxes0_e = torch.cat([padded_boxes0_e, torch.zeros([1, N, 3], device=boxes_g.device)], dim=2)
