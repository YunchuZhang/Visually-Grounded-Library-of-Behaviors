...
class EmbNet2D(nn.Module):
    ...
    def sample_embs(self, emb0, emb1, valid, B, Y, X, mod='', do_vis=False, summ_writer=None):
        if hyp.emb_2D_mindist == 0.0:
            ...
            if do_vis and (summ_writer is not None):
                sample_mask = utils.improc.xy2mask(sample_locs, Y, X)
    ...
    def forward(self, rgb_g, emb_e, valid, summ_writer):
        total_loss = torch.tensor(0.0).cuda()
        
        feat = self.net(rgb_g)
        emb_g = l2_normalize(feat, dim=1)
        valid = torch.round(utils.basic.downsample(valid, 2))
        ...
        total_loss = utils.misc.add_loss('emb2D/emb_2D_ml_loss', total_loss, margin_loss, hyp.emb_2D_ml_coeff, summ_writer)
        ...
        total_loss = utils.misc.add_loss('emb2D/emb_2D_l2_loss', total_loss, emb_l2_loss, hyp.emb_2D_l2_coeff, summ_writer)
        ...
        total_loss = utils.misc.add_loss('emb2D/emb_2D_smooth_loss', total_loss, emb_smooth_loss, hyp.emb_2D_smooth_coeff, summ_writer)
