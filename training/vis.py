...
files = [f"data/franka2/object{i}.npy" for i in range(16)]
for file_id, file in enumerate(files):
    data = np.load(f"data/franka2/object{file_id}.npy",allow_pickle=True).item()
    if "success_class" in data:

        print(file_id, data["success_class"])
    else:
    	print(file_id, data["success_claee"])



import ipdb; ipdb.set_trace()

data = np.load("data/franka/object1.npy",allow_pickle=True).item()
...
for cam_id in range(ncams):
	...
	intrinsic = data["pix_T_cams"][cam_id]
...
# add some trasformation here
visualize(data, bbox_points_from_mesh_adam)
#visualize(data, bbox_points_from_mesh)